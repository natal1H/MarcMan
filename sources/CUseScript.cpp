 ...
 *
 * DESCRIPTION:  Tato funkce p�evede ��slo na string doplnene nulami na delku size v ordinalni podobe napr 005
 ...
 *
 * PARAMETERS:   (IN)	CPtrArray* SourceArray - zdrojov� pole
 *               (OUT)	CPtrArray* NewArray - naklonovan� pole
 *
 * AUTOR:        Jind�ich Stejskal (Jindrich.Stejskal@seznam.cz)
 * 
 * DATE:         1.11.2002
 ...
 *
 * DESCRIPTION:  Serializuje pole pole trid M_PROG_NODE(uzl� programu) do citeln� podoby do souboru
 *
 * PARAMETERS:   (IN)   CPtrArray * aProgNodes - pole trid M_PROG_NODE(uzlu programu)
 *               (IN)	string sFileName - Cesta k souboru do kter� se m� pole serializovat
 *
 * AUTOR:        Jind�ich Stejskal (Jindrich.Stejskal@seznam.cz)
 * 
 * DATE:         1.11.2002
 ...
 *
 * DESCRIPTION:  Serializuje pole trid M_EXPRESSION_ELEMENT(entit v�razu) do citeln� podoby do souboru
 *
 ...
 *
 * DESCRIPTION:  funkce vezme analyzovany text a vyhleda v n�m ��sti za��naj�c� chBegin a kon��c� chEnd a ozna��
 *               je ve stejn� dlouh�m stringu strMark identifikatory chMarkBegin a chMarkEnd. Je-li promenna iInsertField
 *               1 ozna�� se vcetne konecneho chEnd
 *
 * PARAMETERS:   (IN)   string* strAnalyzeText - analyzovany text
 *               (OUT)	string* strMarks - ozna�kovan� text
 *               (IN)	char chBegin - hledan� za��tek
 *               (IN)	char chEnd - hledan� konec
 *               (IN)	char chMarkBegin - znacka zacatku
 *               (IN)	char chMarkEnd - znacka konce
 *               (IN)	int iInsertEnd - m� se vlo�it i konecny znak?
 *
 * AUTOR:        Jind�ich Stejskal (Jindrich.Stejskal@seznam.cz)
 * 
 * DATE:         1.11.2002
 ...
 *
 * DESCRIPTION:  funkce vezme analyzovany text a vyhleda v n�m slovo sSearchString a ozna��
 *               je ve stejn� dlouh�m stringu strMark identifikatory chMarkBegin a chMarkEnd.
 *               Kl��ov� slovo ozna�� pouze v p��pad�, �e se v jeho bezprost�edn�m okol� nach�z�
 *               pouze znaky z mno�iny chNeib.
 *
 * PARAMETERS:   (IN)   string* strAnalyzeText - analyzovany text
 *               (OUT)	string* strMarks - ozna�kovan� text
 *               (IN)	string sSearchString - hledan� za��tek
 *               (IN)	char chMarkBegin - znacka zacatku
 *               (IN)	char chMarkEnd - znacka konce
 *               (IN)	char* chNeib - množina znaku, která se mohou kolem klíčového slova nacházet
 *
 ...
 * FUNCTION:     MarkSigns
 *
 * DESCRIPTION:  funkce vezme analyzovany text a vyhleda v něm retezec *sign(třeba +,-,&&) a označí
 *               je ve stejně dlouhém stringu strMark identifikatory chMarkSign.
 *
 * PARAMETERS:   (IN)   string* strAnalyzeText - analyzovany text
 *               (OUT)	string* strMarks - označkovaný text
 *               (IN)	char *sign- hledaný znaménko
 ...
 *
 * DESCRIPTION:  funkce vezme omarkovan� text tvaru XXXXXXXCXXXMXXXXEXXXXNXX a vypln� v n�m 
 *               sekce za��naj�c� chMarkBegin a kon��c� chMarkEnd charem chMarkBegin.P�i C,N by
 *               vysledek byl XXXXXXXCCCCCCCCCCXXXXNXX
 *
 * PARAMETERS:   (OUT)	string* strMarks - ozna�kovan� text
 *               (IN)	char chMarkBegin - hledan� za��tek
 *               (IN)	char chMarkEnd - hledan� konec
 *
 * AUTOR:        Jind�ich Stejskal (Jindrich.Stejskal@seznam.cz)
 * 
 * DATE:         1.11.2002
 ...
 *
 * DESCRIPTION:  funkce vezme analyzovan� text a tam kde jsou v markovan�m stringu 
 *               znam�nka rovna chMarkSection sma�e jak v analyzovan�m tak markovac�m stringu 
 *
 * PARAMETERS:   (IN)   string* strAnalyzeText - analyzovan� text
 *               (OUT)	string* strMarks - omarkovan� string
 *               (IN)	char chMarkSection - hledan� zna�ka
 *
 * AUTOR:        Jind�ich Stejskal (Jindrich.Stejskal@seznam.cz)
 * 
 * DATE:         1.11.2002
 ...
 *
 * DESCRIPTION:  funkce vezme analyzovaný a omarkovaný text a vytvoří pole analyzovaných
 *               a omarkovaných vzájemně si odpovídajících stringu splitovaných podle jednoho ze znaku v
 *               chLineSeparators
 *
...
void CUseScript::SeparateLines(vector<string *> *arroutLines, vector<string *> *arroutMarkLines,  string* strAnalyzeText, string* strMark, char const * chLineSeparators, char chEmpty)
{
    ...
    while (iCounter<strAnalyzeText->length())
    {
        ...
        if (iDefinitivePossitionStart==(-1)) //��dn� �et�zec nenalezen
            break;
 ...
 *
 * DESCRIPTION:  funkce vr�t� index dal��ho v�skytu sekce typu FindMark v poli ptrMarkElements
 *
 * PARAMETERS:   (IN)    stringArray *ptrMarkElements - markovac� stringy delky jedna
 *               (IN)	 int Index - index od kter�ho se m� za��t
 *               (IN)	 char FindMark - hledan� typ sekce
 *
 ...
 *
 * DESCRIPTION:  funkce spoj� sekce od indexu IndexBegin do indexu IndexEnd a vr�ti je jako retezec
 *
 * PARAMETERS:   (IN)    stringArray *ptrElements - pole sekc�
 *               (IN)	 int IndexBegin - index od kter�ho se m� za��t
 *               (IN)	 int IndexEnd - index konce
 *
 * RETURNED:     string - slou�en� �et�zec
 *
 * AUTOR:        Jind�ich Stejskal (Jindrich.Stejskal@seznam.cz)
 * 
 * DATE:         1.11.2002
 ...
 *
 * DESCRIPTION:  funkce p�id� do pole aProgNodes uzly programu z ��dky programu
 *
 * PARAMETERS:   (OUT)  CPtrArray *aProgNodes - pole sekc�
 *               (IN)   string* strCommand - jedna ��dka k�du
 *               (IN)   string* strMarkCommand - omarkovan� jedna ��dka k�du
 *
 * RETURNED:     0 - OK
 *               1 - FALSE
 *
  ...
  */
    ...
    if ((*(aMarkElements[0]))[0]==BEGIN_OF_DIRECT)
    {
        ...
        if (*(aElements[0])==KEYWORD_IF)
        {
            ...
            if (*(aElements[NextC])!=KEYWORD_THEN) //Nejaky command je ale nen� to Then
            {
            ...
            if (*(newNode->sQuestion)=="") //Pr�zn� ot�zka
            {
            ...
            int NextC2=NextCommand(&aMarkElements,NextC+1,BEGIN_OF_DIRECT); //Hled�m dal�� direct

            if (NextC2!=(-1)) //N�jak� nenalezen 
            {
                if (*(aElements[NextC2])!=KEYWORD_ELSE) //Kdy� to neni else tak blb�
                {    
                    intError=ERR_INVALID_COMBINATION_KEYWORDS;
                    intLineError=aProgNodes->size();
                    return 1;
        ...
        if (*(aElements[0])==KEYWORD_FOR)
        {
            ...
            if (*(aElements[NextC])!=KEYWORD_TO) //Nejaky command je ale nen� to To
            {
                intError=ERR_INVALID_COMBINATION_KEYWORDS;
                intLineError=aProgNodes->size();
                return 1;
            ...
            if (*(newNode->sCommand)=="") //Pr�zn� ot�zka
            {
 ...
 * FUNCTION:     MakeNodes
 *
 * DESCRIPTION:  funkce poslupne vol� funkci NodesFromLines pro ka�d� ��dek k�du a vyhazuje pr�zn� ��dky
 *
 * PARAMETERS:   (OUT)  CPtrArray *aProgNodes - pole uzl� programu
 *               (IN)   stringArray* aLines - pole ��dk� k�du
 *               (IN)   stringArray *aMarkLines - pole omarkovan�ch ��dk� k�du
 *
 * RETURNED:     0 - OK
 *               1 - FALSE
 *
...
/***********************************************************************
 *
 * FUNCTION:     GetPtrNode - ukazatel z pole t��d M_PROG_NODE na t��du o indexu iIndex
 *
 ***********************************************************************/
 ...
 * FUNCTION:     LinkNodes
 *
 * DESCRIPTION:  funkce vezne pole Uzl� programu a propoj� je skoky - tj. vypn� iTrueJump a iFalseJump
 *
 * PARAMETERS:   (OUT)  CPtrArray *aProgNodes - pole uzl� programu
 *
 * RETURNED:     0 - OK
...
/***********************************************************************
 *
 * FUNCTION:     AddNextExpresionElement - p�id� dal�� element do pole v�razu
 *
 ***********************************************************************/
...
/***********************************************************************
 *
 * FUNCTION:     AddNextExpresionIntElement - p�id� dal�� element do pole v�razu
 *
 ***********************************************************************/
 ...
 *
 * DESCRIPTION:  funkce vytvo�� pole element� v�razu sExpression
 *
 * PARAMETERS:   (OUT)  CPtrArray * outarrElements - pole element�(M_EXPRESSION_ELEMENT) v�razu
 *               (OUT)  int* MaxDepth - maxim�ln� nalazen� z�vorkov� vno�en� nap�. ((sd)) by bylo 2            
 *               (IN)  string* sExpression - analyzovan� v�raz
 *
 * RETURNED:     0 - OK
 *               1 - FALSE
 *
...
int CUseScript::AnalyzeExpression(vector<M_EXPRESSION_ELEMENT *>* outarrElements, int* MaxDepth, string* sExpression)
{
    ...
    //Oznacit oper�tory,rovn�tka,z�vorky a carky...
 ...
 *
 * DESCRIPTION:  funkce vr�t� hodnotu promenn� ulo�en�ch v asociativn�m poli gVariables
 *
 ...
 *
 * DESCRIPTION:  funkce nastav� hodnotu promenn� ulo�en�ch v asociativn�m poli gVariables
 *               v p��pad� �e neexistuje, zalo�� ji a d� ji defaultn� hosnotu.
 *
 * PARAMETERS:   (IN)  string ValueName - element v jeho� sName je nazev promenne
 *               (IN)  M_EXPRESSION_ELEMENT* pValue - element v nem� je ulo�ena nov� hodnota
 *
 * RETURNED:     0 - OK
 *               1 - FALSE
 *
 ...
 *
 * DESCRIPTION:  funncke provede operaci mezi One Two a vysledek ulo�� do Two
 *
 ...
 *
 * DESCRIPTION:  funkce provede v�po�et sekc� mezi BeginIndex a EndIndex pomoc� p�ru oper�tor� nap� +,-
 *
 * PARAMETERS:   (IN)  CPtrArray * outarrElements - pole element� v�razu
 *               (IN)  int BeginIndex 
 *               (IN)  int EndIndex
 *               (IN)  char *Operator1
 *               (IN)  char *Operator2
 *               (OUT) int *Removed - kolik zmizelo element� v�po�tem
 *
 * RETURNED:     0 - OK
 *               1 - FALSE
 *
 ...
 *
 * DESCRIPTION:  funkce provede v�po�et sekce mezi BeginIndex a EndIndex
 *               podle v�ech oper�toru a vol� fukci SectionCalculateByOperator 
 ...
 * FUNCTION:     CallMethod
 *
 * DESCRIPTION:  funkce vol� ud�lost v klientovi komponenty a p�ed�v� mu kolekci
 *               parametr�. N�vratovou hodnotu v prvn�m prvku kolekce vrac� jako
 *               M_EXPRESSION_ELEMENT
 *
 * PARAMETERS:   (IN)  string MethodName - n�zev volan� funkce
 *               (IN)  CPtrArray * arguments - pole argument� funcke
 *
 * RETURNED:     M_EXPRESSION_ELEMENT hodnota vr�cen� funkc�
 *
 * AUTOR:        Jind�ich Stejskal (Jindrich.Stejskal@seznam.cz)
 * 
 * DATE:         1.11.2002
 ...
 *
 * DESCRIPTION:  funkce vyhled� funkce v sekc�ch mezi BeginIndex a EndIndex, zavol� je
 *               a v�sledek za�ad� do pole element� 
 *
 * PARAMETERS:   (IN/OUT)  CPtrArray * outarrElements - pole element� v�razu
 *               (IN)      int BeginIndex 
 *               (IN)      int EndIndex
 *               (OUT)     int *Removed - kolik zmizelo element� v�razu
 *
 * RETURNED:     0 - OK
 *               1 - FALSE
 *
 ...
 *
 * DESCRIPTION:  funkce vyhled� dal�� skupinu sekc� k anal�ze v aktu�ln� hloubce v�razu. Jesli�e
 *               ji� ��dnou sekci s hloubkou  iDepth nenajde vrac� nula
 *
 * PARAMETERS:   (IN/OUT)  CPtrArray * outarrElements - pole element� v�razu
 *               (IN)      int iDepth - hledan� hloubka 
 *               (OUT)     int* BeginSection - zac�tek nalezen� sekce
 *               (OUT)     int* EndSection- konec nalezen� sekce
 *
 * RETURNED:     0 - sekce nalezena
 *               1 - ��dn� sekce v hloubce iDepth nejsou
 *
 * AUTOR:        Jind�ich Stejskal (Jindrich.Stejskal@seznam.cz)
 * 
 * DATE:         1.11.2002
 ...
 *
 * DESCRIPTION:  funkce analyzuje cel� v�raz. Bu� je jej�m v�stupem jeden element (��slo nebo
 *               �et�zec nebo jsou jej�m v�stupem t�i elementy pak je to p�i�azovac� v�raz
 *               (a=2)
 *
 * PARAMETERS:   (IN/OUT)  CPtrArray * outarrElements - pole element� v�razu
 *               (IN)      int iDepth - nejvets� hloubka po anal�ze - uchov�van� v PROG_NODES 
 *
 * RETURNED:     0 - OK
 *               1 - FALSE
 *
...
int CUseScript::ProcessExpression(vector<M_EXPRESSION_ELEMENT *> *outarrElements, int iDepth)
{
    //int IsComma;
    int IsAssign=0;
    int BeginSection = 0; // Iba na odstránenie warningu, hodnota sa priradí v GetNextSection()
    int EndSection;
 ...
 *
 * DESCRIPTION:  funkce provede anal�zu v�ech v�raz� v programu a ulo�� je do pol� CommandExpression
 *               a QuestionExpression
 *
 * PARAMETERS:   (IN/OUT)  CPtrArray *aProgNodes - pole uzl� programu
 *
 * RETURNED:     0 - OK
 *               1 - FALSE
 *
